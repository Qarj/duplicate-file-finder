...
def dff(path, delete_duplicates=False):
    output('\n' + time.strftime('%X : ') +
           'Finding duplicate files at ' + path + '\n')
    ...
    for current_file_path in sizes.files_list:
        ...
        if (current_file_snip_hash in snip):
            dupe_file_path = full_hash.search_duplicate(
                snip[current_file_snip_hash], current_file_path)
            if (dupe_file_path):
                display_duplicate_and_optionally_delete(
                    dupe_file_path, current_file_path, delete_duplicates)
                duplicate_count += 1
            else:
    ...
    if (failed_delete_count):
        output('\n' + 'failed to delete ' +
               str(failed_delete_count) + ' duplicates - rerun script')
...
class fileSizes:
    ...
    def add_file(self, current_file_path, size):
        if size in self.sizes:
            verbose(current_file_path +
                    ' has non unique file size [' + str(size) + ' bytes]')
...
def display_duplicate_and_optionally_delete(previously_hashed_file_path, current_file_path, delete_duplicates):
    ...
    output(current_file_message + current_file_path + '\n is dupe of ' +
           previously_hashed_file_path + previously_hashed_file_message + '\n')
...
def delete_duplicate_and_get_message(previously_hashed_file_path, current_file_path):
    ...
    if (not trial_delete):
        try:
            ...
            # only the previously hashed file could have been deleted - not the current file (unless user is deleting files outside of this script!)
...
parser = argparse.ArgumentParser(
    description='Find duplicate files in target path and sub folders.')
...
parser.add_argument('--delete', action='store_true', dest='delete',
                    default=False, help='Deletes any duplicate files found')
parser.add_argument('--trial', action='store_true', dest='trial_delete', default=False,
                    help='Displays files to delete without actually deleting them - use with --delete')
parser.add_argument('--shorter', action='store_true', dest='delete_shorter', default=False,
